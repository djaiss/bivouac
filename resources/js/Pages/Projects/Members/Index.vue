  ...
  <AuthenticatedLayout>
    <div class="mx-auto mb-6 mt-8 max-w-7xl space-y-6 px-12 sm:px-6 lg:px-8">
      ...
      <div class="mx-auto flex max-w-2xl bg-white px-4 py-2 text-sm shadow sm:rounded-lg">
        ...
        <p>
          {{ $t('If the project is public, anyone can take part without having to be a formal member. On the other hand, private projects are only accessible and open to members.') }}
        </p>
      </div>
      ...
      <div v-if="potentialMembersShown" class="mx-auto max-w-2xl bg-white shadow sm:rounded-lg">
        ...
        <div class="flex items-center justify-between border-b px-4 py-2">
          ...
          <XMarkIcon @click="potentialMembersShown = false" class="h-5 w-5 cursor-pointer rounded text-gray-400 hover:bg-gray-300 hover:text-gray-600 group-hover:block" />
        </div>
        <div v-if="loadingUsers" class="p-3 text-center">{{ $t('Loading users...') }}</div>

        <!-- list -->
        <ul class="max-h-40 overflow-auto">
          <li v-for="user in localPotentialMembers" :key="user.id" class="flex items-center justify-between py-4 pl-4 pr-6 hover:bg-slate-50 last:hover:rounded-b-lg">
            <!-- user name -->
            <div>{{ user.name }}</div>

            <!-- add button -->
            <span @click="store(user)" class="flex cursor-pointer rounded-md border border-gray-300 bg-gray-100 px-3 py-1 font-semibold text-gray-700 hover:border-solid hover:border-gray-500 hover:bg-gray-200">
              {{ $t('Add') }}
            </span>
          </li>
        </ul>
      ...
      <div class="mx-auto max-w-2xl bg-white shadow sm:rounded-lg">
        ...
        <ul v-if="localMembers.length > 0" class="w-full">
          <li v-for="member in localMembers" :key="member.id" class="flex items-center justify-between py-4 pl-4 pr-6 hover:bg-slate-50 last:hover:rounded-b-lg">
            ...
            <Menu v-if="page.props.auth.user.id !== member.id" as="div" class="icon-menu relative z-30 text-left">
              ...
              <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                <MenuItems class="absolute right-0 z-50 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div class="bg-white px-1 py-1">
                    <MenuItem v-slot="{ active }">
                      <button @click="remove(member)" :class="[active ? 'bg-violet-500 text-white' : 'text-gray-900', 'group flex w-full items-center rounded-md px-2 py-2 text-sm']">
                        {{ $t('Remove') }}
                      </button>
                    </MenuItem>
                  </div>
