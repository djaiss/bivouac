        ...
        <div>
          ...
          <div class="relative mb-4 bg-white shadow sm:rounded-lg">
            ...
            <div class="border-b px-6 py-6">
              <!-- task header -->
              <div v-if="!editTitleShown" class="flex cursor-pointer items-start rounded-lg border border-transparent px-2 py-2 hover:border-gray-200">
                <Checkbox @click="toggleTask()" :checked="task.is_completed" :name="'completed' + task.id" class="checkbox-title relative mr-3 h-6 w-6" />
              ...
              <div v-if="editTitleShown">
                <form @submit.prevent="update()" class="flex items-center justify-between">
                  <TextInput id="term" type="text" :placeholder="$t('Enter a title')" class="mr-3 w-full" v-model="form.title" autofocus @keydown.esc="editTitleShown = false" required />
                  ...
                  <div class="flex items-center">
                    ...
                    <span @click="editTitleShown = false" class="flex cursor-pointer rounded-md bg-gray-100 px-3 py-1.5 font-semibold text-gray-700 shadow-sm ring-1 ring-inset ring-gray-200 hover:bg-gray-200">
                      {{ $t('Cancel') }}
                    </span>
                  </div>
                </form>
              ...
              <div v-if="!description && !editDescriptionShown" @click="editDescription()" class="mt-4 cursor-pointer text-sm text-gray-600 hover:underline">
                {{ $t('+ add description') }}
              </div>
              ...
              <form v-if="editDescriptionShown" @submit.prevent="update()" class="mt-6">
                ...
                <div v-if="activeTab === 'write'">
                  <TextArea @esc-key-pressed="editDescriptionShown = false" id="description" ref="bodyInput" class="block w-full" required autogrow v-model="form.body" />
                  ...
                  <div v-if="form.body" class="mt-4 flex justify-start">
                    ...
                    <span @click="editDescriptionShown = false" class="flex cursor-pointer rounded-md border border-gray-300 bg-gray-100 px-3 py-1 font-semibold text-gray-700 hover:border-solid hover:border-gray-500 hover:bg-gray-200">
                      {{ $t('Cancel') }}
                    </span>
                  </div>
                </div>
          ...
          <div class="rounded-lg shadow">
            ...
            <div class="rounded-t-lg border-b bg-white px-6 py-4">
              ...
              <div v-if="task.assignees.length > 0">
                <div v-for="assignee in localAssignees" :key="assignee.id" class="group mb-4 flex items-center justify-between rounded-lg px-2 py-1 hover:bg-gray-100">
                  <div class="flex items-center">
                    <Avatar v-tooltip="assignee.name" :data="assignee.avatar" :url="assignee.url" class="h-6 w-6 cursor-pointer rounded-full" />

                    <Link :href="assignee.url" class="ml-2 text-sm text-blue-700 underline hover:rounded-sm hover:bg-blue-700 hover:text-white">
                      {{ assignee.name }}
                    </Link>
                  </div>

                  <XMarkIcon @click="unassign(assignee)" class="hidden h-5 w-5 cursor-pointer rounded text-gray-400 hover:bg-gray-300 hover:text-gray-600 group-hover:block" />
                </div>
              </div>

              <!-- links to add assignees -->
              <ul class="text-sm">
                <li @click="showSearch()" v-if="!searchShown" class="mr-2 inline cursor-pointer text-gray-600 hover:underline">
                  {{ $t('Add someone') }}
                </li>
                <li @click="assign(loggedUser)" v-if="!isSelfAssigned" class="inline cursor-pointer text-gray-600 hover:underline">
                  {{ $t('Assign yourself') }}
                </li>
              </ul>
              ...
              <div v-if="searchShown">
                <!-- search field -->
                <div class="relative">
                  <TextInput type="text" autocomplete="off" :placeholder="$t('Type a few letters')" class="mt-2 block w-full" v-model="form.term" autofocus @input="searchUsers()" @keydown.esc="searchShown = false" required />
