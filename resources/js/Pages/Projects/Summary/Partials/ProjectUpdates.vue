      ...
      <div class="mb-4 bg-white shadow sm:rounded-lg">
        ...
        <div class="flex items-center justify-between border-b px-4 py-4">
          <div class="group flex items-center">
            <Avatar v-tooltip="projectUpdate.author.name" :data="projectUpdate.author.avatar" :url="projectUpdate.author.url" class="mr-4 h-10 w-10 rounded" />

            <div>
              <Link :href="projectUpdate.author.url" class="text-sm text-gray-600 group-hover:text-blue-700 group-hover:underline">
              ...
              <MenuItems class="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <div class="px-1 py-1">
                  <MenuItem v-slot="{ active }">
                    <span @click="showEditUpdate(projectUpdate)" :class="[active ? 'bg-violet-500 text-white' : 'text-gray-900', 'group flex w-full items-center rounded-md px-2 py-2 text-sm']">
                      {{ $t('Edit') }}
                    </span>
                  </MenuItem>
                  <MenuItem v-slot="{ active }">
                    <button @click="destroy(projectUpdate)" :class="[active ? 'bg-violet-500 text-white' : 'text-gray-900', 'group flex w-full items-center rounded-md px-2 py-2 text-sm']">
                      {{ $t('Delete') }}
                    </button>
                  </MenuItem>
                </div>
        ...
        <div v-else class="px-4 py-4">
          <form @submit.prevent="update(projectUpdate)">
            <div class="mb-4">
              ...
              <!-- write mode -->
              <div v-if="activeTab === 'write'">
                <TextArea id="description" class="block w-full" required v-model="form.body" @esc-key-pressed="editedUpdateId = 0" />
              </div>
            ...
            <div class="mt-4 flex justify-start">
              ...
              <span @click="editedUpdateId = 0" class="flex cursor-pointer rounded-md border border-gray-300 bg-gray-100 px-3 py-1 font-semibold text-gray-700 hover:border-solid hover:border-gray-500 hover:bg-gray-200">
                {{ $t('Cancel') }}
              </span>
            </div>
          </form>
