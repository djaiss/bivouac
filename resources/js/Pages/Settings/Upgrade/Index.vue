        ...
        <div class="w-full">
          <div v-if="!isUpgraded">
            ...
            <div class="mb-14 grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
              <div class="bg-white p-8 shadow sm:rounded-lg">
                ...
                <div class="mb-6 flex flex-col rounded-lg border border-gray-100 bg-gray-100 p-10 text-center">
                  ...
                  <a :href="data.url.store" target="_blank" class="rounded-md bg-indigo-500 px-3 py-1.5 text-center font-semibold text-white shadow-sm ring-1 ring-inset ring-indigo-600 hover:bg-indigo-700">
                    {{ $t('Unlock') }}
                  </a>
                </div>
            ...
            <form @submit.prevent="update()" class="mx-auto mb-14 max-w-2xl rounded-lg bg-white p-8 shadow">
              <div class="mb-4">
                <InputLabel for="licence_key" :value="$t('Grab a licence key above, and paste it here to unlock your account')" class="mb-1" />

                <TextInput id="licence_key" type="text" class="block w-full text-center" v-model="form.licence_key" autofocus required />
              </div>
            ...
            <div class="px-14">
              ...
              <p class="mb-6">
                {{ $t("For a single fee, you'll get access to Bivouac for life – no need to pay more than once.Pay now and enjoy it for years to come.To be crystal clear, there's no ongoing subscription required.") }}
              </p>

              <p class="mb-3 text-xl font-bold">{{ $t("Isn't that too expensive?") }}</p>
              <p class="mb-6">
                {{ $t('Most software packages require a subscription, and the cost is based on the number of users. If your software costs $100 a month, then it will cost you $1,200 a year. Our deal seems like a pretty good deal.') }}
              </p>

              <p class="mb-3 text-xl font-bold">{{ $t('What does "Free updates" mean?') }}</p>
              <p class="mb-6">
                {{ $t('It means that all updates, including new features and bug fixes, will be available to you at no extra cost, forever.') }}
              </p>

              <p class="mb-3 text-xl font-bold">{{ $t('What is your refund policy?') }}</p>
              <p class="mb-6">
                {{ $t("If you're unhappy with your purchase for any reason, please send us an email within 30 days and we'll refund you in full. After those 30 days, we will not issue a refund.") }}
              </p>
            </div>
          </div>
