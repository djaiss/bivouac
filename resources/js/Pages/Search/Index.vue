        ...
        <div class="w-full">
          ...
          <div class="mb-10 rounded-lg bg-white shadow">
            <form @submit.prevent="submit" class="flex items-center justify-between p-3">
              <TextInput id="term" type="text" :placeholder="$t('Search anything')" class="mr-3 w-full" v-model="form.term" autofocus required />
          ...
          <div v-if="results.length !== 0">
            ...
            <ul v-if="results.users.length > 0" class="w-full rounded-lg bg-white shadow">
              <li v-for="user in results.users" :key="user.id" class="group flex items-center justify-between px-6 py-4 hover:bg-slate-50 first:hover:rounded-t-lg last:hover:rounded-b-lg">
                <!-- user information -->
                <div class="flex items-center">
                  ...
                  <div class="mr-6 flex flex-col">
                    <div>
                      <Link :href="user.url.show" class="text-blue-700 underline hover:rounded-sm hover:bg-blue-700 hover:text-white">
                        {{ user.name }}
                      </Link>
                    </div>
                    <div class="flex">
            ...
            <ul v-if="results.projects.length > 0" class="w-full rounded-lg bg-white shadow">
              <li v-for="project in results.projects" :key="project.id" class="group flex items-center justify-between px-6 py-4 hover:bg-slate-50 first:hover:rounded-t-lg last:hover:rounded-b-lg">
                <div class="flex items-center">
                  <div>
            ...
            <ul v-if="results.messages.length > 0" class="w-full rounded-lg bg-white shadow">
              <li v-for="message in results.messages" :key="message.id" class="group flex items-center justify-between px-6 py-4 hover:bg-slate-50 first:hover:rounded-t-lg last:hover:rounded-b-lg">
                <div class="flex items-center">
                  <div>
                    <Link :href="message.url.show" class="text-blue-700 underline hover:rounded-sm hover:bg-blue-700 hover:text-white">
                      {{ message.title }}
                    </Link>
                  </div>
                </div>
