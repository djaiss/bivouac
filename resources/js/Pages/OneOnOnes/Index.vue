        ...
        <div class="w-full">
          <div class="bg-white shadow sm:rounded-lg">
            ...
            <ul v-if="props.data.one_on_ones.length > 0" class="rounded-lg border bg-white dark:bg-gray-900">
              <li v-for="oneOnOne in props.data.one_on_ones" :key="oneOnOne.id" class="border-b border-gray-200 px-3 py-4 last:border-0 hover:bg-slate-50 first:hover:rounded-t-lg last:hover:rounded-b-lg dark:border-gray-700 dark:bg-slate-900 hover:dark:bg-slate-800">
                <div v-if="page.props.auth.user.id === oneOnOne.user.id" class="flex items-center justify-between">
                  <div class="flex items-center">
                    <Avatar :data="oneOnOne.other_user.avatar" class="mr-2 w-8" />

                    {{ oneOnOne.other_user.name }}
                  </div>
                  <Link :href="oneOnOne.url.show" class="inline text-blue-700 underline hover:rounded-sm hover:bg-blue-700 hover:text-white">
                    {{ $t('Participate') }}
                  </Link>
                </div>
                ...
                <div v-if="page.props.auth.user.id !== oneOnOne.user.id" class="flex items-center justify-between">
                  <div class="flex items-center">
                    <Avatar :data="oneOnOne.user.avatar" class="mr-2 w-8" />
